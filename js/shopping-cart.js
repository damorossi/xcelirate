...
function addToShoppingCart(productId, qty) {
  const existingCartProduct = storeModule.store.cart.products.find(
    ({ product }) => product.id === productId
  );
  ...
  if (!existingCartProduct) {
    ...
    let newCartProducts = storeModule.store.cart.products.filter(
      (item) => item.product.id !== productId
    );
    const item = {
...
function getCostWithNoDiscount() {
  let totalCost = 0;
  for (let i = 0; i < storeModule.store.cart.products.length; i += 1) {
    totalCost += storeModule.store.cart.products[i].product.price
      * storeModule.store.cart.products[i].quantity;
  }
  return totalCost;
}
